...
namespace RecolorImageWithSpot;
...
internal class RecolorImageWithSpot
{
    ...
    static int Main()
    {
        try
        {
            ...
            using var rubineRed = MakeSeparationColor(mako, "Pantone Rubine Red C", [43.92, 79.0, 13.0]);
            using var rubineRedBrush = IDOMSolidColorBrush.create(mako, rubineRed);
            ...
            using var imageColorSpace = imageFrame.getColorSpace();
            ...
            using var imageFilters = new CEDLVectIDOMImageFilter();
            ...
            using var mask = IDOMFilteredImage.create(mako, image, imageFilters);

            // Create a masked brush
            using var maskedBrush = IDOMMaskedBrush.create(mako, mask, rubineRedBrush, imageBounds, imageBounds);

            // Create a path of suitable size and paint with masked brush
            using var path = IDOMPathNode.createFilled(mako, IDOMPathGeometry.create(mako, imageBounds), maskedBrush);
            ...
            // Save to PDF
            IPDFOutput.create(mako).writeAssembly(assembly, IOutputStream.createToFile(mako, "output.pdf"));
        }
    ...
    private static IDOMColor MakeSeparationColor(IJawsMako mako, string name, double[] labRepresentation)
    {
        ...
        var alternate = IDOMColorSpaceLAB.create(mako, 0.9642f, 1.0000f, 0.8249f, 0.0f, 0.0f, 0.0f, -128f, 127f,
            -128f, 127f);
        var colorSpace = IDOMColorSpaceDeviceN.create(mako.getFactory(), colorants, alternate.toIDOMColorSpace());
    ...
    private static IDOMColor MakeCmykSeparationColor(IJawsMako mako, string name, double[] cmykRepresentation)
    {
        // Create a vector of colorants with one entry
