...
class CustomTextTransform
{
    ...
    static int Main(string[] args)
    {
        ...
        var inkValue = float.Parse(args[2]);
        ...
        try
        {
            ...
            using var implementation = new TextTransformImplementation(jawsMako, inkValue);

            using var textModifier = ICustomTransform.create(
                jawsMako,
                implementation
            );

            using var assembly = IInput.create(jawsMako, eFileFormat.eFFPDF).open(TestFilesPath + inputFile);
            using var document = assembly.getDocument();

            var pageCount = document.getNumPages();
