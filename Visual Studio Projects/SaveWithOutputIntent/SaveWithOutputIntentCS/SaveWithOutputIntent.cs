...
class SaveWithOutputIntent
{
    ...
    static int Main()
    {
        ...
        {
            Tuple.Create("Probev2_ICCv4.icc", "https://www.color.org/probeprofile.xalter"),
            Tuple.Create("GRACoL2006_Coated1v2.icc", "https://idealliance.org"),
            Tuple.Create("JapanColor2011Coated.icc", "https://www.xrite.com")
        };
        ...
        try
        {
            ...
            using var fixedPage = IDOMFixedPage.create(mako, M2X(250), M2X(210));
            page.setContent(fixedPage);

            // Get a font
            using var font = IDOMFontOpenType.fromRCObject(mako.findFont("Arial", out var fontIndex).toRCObject());

            // Create a colour
            using var darkBlue = IDOMColor.createSolidRgb(mako, 0.0f, 0.0f, 0.5f);
            ...
            using var header = ILayoutParagraph.create(ILayoutParagraph.eHorizontalAlignment.eHALeft, P2X(6));
            using var body = ILayoutParagraph.create(ILayoutParagraph.eHorizontalAlignment.eHALeft, P2X(5));

            // A vector to point to each paragraph to be added to the frame(s)
            using var paragraphs = new CEDLVectILayoutParagraph();
            uint paraIndex = 0;
            ...
            // Picture
            using var parrot = GetImage(ref mako, TestFilesPath + "Parrot.png");
            ...
            using var iccProfile = IDOMICCProfile.create(mako, IInputStream.createFromFile(mako, Path.Combine(TestFilesPath, profiles[0].Item1)));
            ...
            using var outputIntents = new CEDLVectIOutputIntent();
    ...
    private static double P2X(double value) => value / 72.0 * 96.0;

    // Millimetres to XPS units (1/96")
    private static double M2X(double value) => value / 25.4 * 96.0;
    ...
    private static IDOMImage GetImage(ref IJawsMako mako, string imageFile)
    {
        ...
        return image == null ? throw new Exception($"Image file {imageFile} could not be loaded.") : image;
    }
}